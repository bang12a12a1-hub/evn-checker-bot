...
class SPCProvider(BaseEVNProvider):
    """Provider for EVNSPC (21 Tỉnh Miền Nam) - Code prefixes: PC, PS"""
    ...
    def check(self, customer_code: str) -> BillCheckResult:
        ...
        try:
            url = "https://cskh.evnspc.vn/TraCuu/GetTienDien"
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        return BillCheckResult(
                            ...
                            bills=bills,
                            raw_message="Tra cứu thành công từ EVNSPC"
                        )
                except Exception:
                    html = resp.text
                    if "không có dư nợ" in html.lower() or "đã thanh toán" in html.lower() or "không tìm thấy" in html.lower():
                        return BillCheckResult(
                            ...
                            total_debt=0.0,
                            bills=[],
                            raw_message="Đã thanh toán (Không có dư nợ)"
                        )
        except Exception:
            pass
        ...
        return BillCheckResult(
            ...
            success=False,
            is_paid=True,
            error="Không thể truy cập dịch vụ EVNSPC"
        )
    ...
    def _check_via_playwright(self, customer_code: str) -> BillCheckResult:
        ...
        try:
            with sync_playwright() as p:
                ...
                page.goto("https://cskh.evnspc.vn/TraCuu/TraCuuTienDien", timeout=15000)
            ...
            return BillCheckResult(
                ...
                success=False,
                is_paid=True,
                error=f"Lỗi Playwright EVNSPC: {str(e)}"
            )
