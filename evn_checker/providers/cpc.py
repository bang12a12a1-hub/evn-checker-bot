...
class CPCProvider(BaseEVNProvider):
    """Provider for EVNCPC (Miền Trung & Tây Nguyên) - Code prefixes: PP, PQ, PK"""
    ...
    def check(self, customer_code: str) -> BillCheckResult:
        ...
        try:
            url = "https://cskh.cpc.vn/api/tracuu/tiendien"
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        return BillCheckResult(
                            ...
                            bills=bills,
                            raw_message="Tra cứu thành công từ EVNCPC"
                        )
                except Exception:
                    html = resp.text
                    if "không có dư nợ" in html.lower() or "đã thanh toán" in html.lower():
                        return BillCheckResult(
                            customer_code=customer_code,
                            region=self.region,
                            success=True,
                            ...
                            total_debt=0.0,
                            bills=[],
                            raw_message="Đã thanh toán (Không nợ tiền điện)"
                        )
        except Exception:
            pass
        ...
        return BillCheckResult(
            ...
            success=False,
            is_paid=True,
            error="Không thể truy cập EVNCPC"
        )
    ...
    def _check_via_playwright(self, customer_code: str) -> BillCheckResult:
        ...
        try:
            with sync_playwright() as p:
                ...
                page.goto("https://cskh.cpc.vn/tra-cuu/tiendien", timeout=15000)
            ...
            return BillCheckResult(
                ...
                success=False,
                is_paid=True,
                error=f"Lỗi Playwright EVNCPC: {str(e)}"
            )
